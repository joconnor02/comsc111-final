                                        ...
                                        implements BinarySearchTreeADT<T>
{
    ...
    private BinaryTreeNode<T> findNode(T targetElement, BinaryTreeNode<T> next) 
    {
    	...
    	//get the right child, greater than current, as the next node
    	BinaryTreeNode<T> newNode = findNode(targetElement, next.getRight());
    	
    	//If there is no right node or target is less than right, go left
    	if(newNode == null || comparableElement.compareTo(newNode.getElement()) < 0)
    	{
        	return findNode(targetElement, next.getLeft());
    	}
    	
    	//Go right if it is less than or equal to target, current is not null or the target
    	return findNode(targetElement, newNode);
    }
}
